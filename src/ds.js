...
const squareStep = (heights, i, getDisplacement) => {
  ...
  for (let y = 0; y < SIDE; y += halfOffset) {
    for (let x = y % offset === 0 ? halfOffset : 0; x < SIDE; x += offset) {
      ...
      heights[ci] = v + getDisplacement(i);
    }
  }
...
const diamondStep = (heights, i, getDisplacement) => {
  const offset = getDiamondOffset(SIDE, i);
  const halfOffset = offset / 2;
  const maxXY = SIDE - 1;
...
const generateMap = (getRandom, roughness) => {
  ...
  const getDisplacement = iteration =>
    (2 * roughness * getRandom() - roughness) ** (iteration + 1);

  for (let i = 0; i < ITERATIONS; i++) {
    diamondStep(heights, i, getDisplacement);
    squareStep(heights, i, getDisplacement);
  }
